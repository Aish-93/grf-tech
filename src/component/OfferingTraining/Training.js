import React from 'react';
import './Training.css';
import email from '../../icons/email.jpg';

const Training = () => {
  return (
    <div className='mt-5'>
      <h3 className='verticle-heading text-5xl font-semibold'>We Provide </h3>
      <div className='d-flex justify-content-around mt-5 mx-5 px-5'>
        <div className='ani-card'>
          <img
            src='https://img.freepik.com/free-photo/view-office-desk-with-messy-workspace-laptop_23-2150282081.jpg?uid=R139239883&ga=GA1.1.1110932627.1730523152&semt=ais_hybrid'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Email Writing</h5>
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}
            <br />
            <p className='ani-card-info'>
              Learn to write clear, professional, and impactful emails for
              effective communication and lasting impressions.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
        <div className='ani-card'>
          <img
            src='https://img.freepik.com/free-photo/diagram-success_1232-206.jpg?uid=R139239883&ga=GA1.1.1110932627.1730523152&semt=ais_hybrid'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Complex Logical Approach</h5>
            <br />
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}
            <p className='ani-card-info'>
              We teach a logical and structured approach to solving complex
              coding problems and scenarios.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
        <div className='ani-card'>
          <img
            src='https://img.freepik.com/free-photo/preteen-girl-shirt-jacket-pointing-camera-looking-optimistic_176474-19517.jpg?uid=R139239883&ga=GA1.1.1110932627.1730523152&semt=ais_hybrid'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Easy to Understand</h5>
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}
            <br />
            <p className='ani-card-info'>
              We simplify complex coding concepts, making them easy to
              understand and apply effectively.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
        <div className='ani-card'>
          <img
            src='https://img.freepik.com/free-photo/smiling-business-leader-greeting-partner_1262-3306.jpg?uid=R139239883&ga=GA1.1.1110932627.1730523152&semt=ais_hybrid'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Formal Communication</h5>
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}
            <br />
            <p className='ani-card-info'>
              We provide training to master formal communication skills for
              professional and impactful interactions.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
      </div>
      <div className='d-flex justify-content-around pb-5 my-5 mx-5 px-5'>
        <div className='ani-card'>
          <img
            src='https://img.freepik.com/free-photo/colleagues-learning-together-group-study_23-2149211095.jpg?t=st=1731694349~exp=1731697949~hmac=e1990e1110b483c500e69ebc16869ae7f361b03a0eb784fdaf73e844ea206526&w=996'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Internship</h5>
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}
            <br />
            <p className='ani-card-info'>
              We offer internships to help you gain real-world coding experience
              and apply your skills in practical scenarios.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
        <div className='ani-card'>
          <img
            src='https://img.freepik.com/free-photo/businessmen-using-touchpad-meeting_1098-233.jpg?uid=R139239883&ga=GA1.1.1110932627.1730523152&semt=ais_hybrid'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Mock Interviews</h5>
            <br />
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}

            <p className='ani-card-info'>
              We conduct mock interviews to prepare you for real-world job
              interviews with confidence and expertise.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
        <div className='ani-card'>
          <img
            src='https://img.freepik.com/free-photo/education-diploma-certificate-held-by-wooden-dummy_23-2148769651.jpg?uid=R139239883&ga=GA1.1.1110932627.1730523152&semt=ais_hybrid'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Certification</h5>
            <br />
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}
            <p className='ani-card-info'>
              We offer certifications to showcase your expertise and boost your
              professional credibility in the tech field.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
        <div className='ani-card'>
          <img
            src='https://w0.peakpx.com/wallpaper/140/145/HD-wallpaper-boudhanath-amoled-boudha-cultural-design-nature-nepal-patterns-peace-spiritual-temple.jpg'
            className='ani-card-img'
            alt=''
          />
          <div className='ani-card-body'>
            <h5 className='ani-card-title'>Economics</h5>
            <br />
            {/* <p className='ani-card-sub-title'>Economic Ties</p> */}
            <p className='ani-card-info'>
              Economics time between india and nepal is splendid Lorem ipsum
              lorem ipsum dolor sit amet.
            </p>
            {/* <button className='ani-card-btn'>View More</button> */}
          </div>
        </div>
      </div>
      {/* <div class='heading-frame'>
        <h2 contenteditable='true'>
          YOU CAN
          <br />
          EDIT ME, JUST TYPE
        </h2>
      </div> */}
    </div>
  );
};

export default Training;
